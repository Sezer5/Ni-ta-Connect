...
<div class="main-content">
    <div class="card border-0 shadow-sm rounded-4 p-4">
        <h2 class="fw-bold mb-1" style="color: var(--nigtas-blue);font-family:Pirulen">AÇIK İRSALİYELER</h2>
        <hr>
        <div class="table-responsive"> <!-- Mobilde taşmaması için eklendi -->
            <table id="irsaliyeTable" class="table table-striped table-bordered w-100">
                ...
                <tbody>
                    <?php if(!empty($irsaliye_verileri)): ?>
                        <?php foreach($irsaliye_verileri as $satir): ?>
                            <tr>
                                <td><?= date('d.m.Y', strtotime($satir['Tarih'])) ?></td>
                                <td><?= $satir['IRSALIYE_NO'] ?></td>
                                <td><?= $satir['MAMUL'] ?></td>
                                <td><?= $satir['VADE'] ?></td>
                                <td><?= $satir['TONAJ'] ?></td>
